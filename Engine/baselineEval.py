...
def eval(stations):
    """
    Calculates the TCN model's performance on the test set for each station. These metrics are written to a file
    ...
    for station in stations:
        for forecast_len in [3, 6, 9, 12, 24]:
            yhat = 'Results/TCN/' + str(forecast_len) + ' Hour Forecast/' + station + '/Predictions/' + \
                   'result.csv'
            target = 'Results/TCN/' + str(forecast_len) + ' Hour Forecast/' + station + '/Targets/' + \
                     'target.csv'
            metricFile = 'Results/TCN/' + str(forecast_len) + ' Hour Forecast/' + station + '/Metrics/' + \
                         '/metrics.txt'
