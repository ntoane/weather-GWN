...
class StandardScaler:
    """
    Creates a scaler object that performs z-score normalisation on a data set
    """
...
def sliding_window(df, lag, forecast, split, set):
    ...
    """
    if set == 0:
        samples = int(split[0] / 27)
    if set == 1:
        samples = int(split[1] / 27 - split[0] / 27)
    if set == 2:
        samples = int(split[2] / 27 - split[1] / 27)


    dfy = df.drop(['Rain', 'Humidity', 'Pressure', 'WindSpeed', 'WindDirection'], axis=1)
    stations = 27
    features = 6
