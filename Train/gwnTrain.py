...
def train_model(args, data_sets, split, supports, adj_init, dictionary):
    ...
    for epoch in range(args.epochs):
        ...
        print(
            'Epoch {:2d} of {:2d} | Train Time: {:4.2f}s | Train Loss: {:5.4f} | Validation Time: {:5.4f} | Validation Loss: '
            '{:5.4f} '.format(
                epoch + 1, args.epochs, trainTime, train_loss, validationTime, validation_loss))

        # patience is used for early stopping
...
def train(increment, args):
    ...
    data = data.drop(['StasName', 'Latitude', 'Longitude', 'DateT'], axis=1)
    ...
    for forecast_len in forecast_horizons:
        ...
        for k in range(args.n_split):
            print('Training WGN models through walk-forward validation on a forecasting horizon of: ', args.seq_length, ', Split: ', k)
            ...
            fileDictionary = {'predFile': 'Results/GWN/' + str(forecast_len) + ' Hour Forecast/Predictions/outputs_' +
                              ...
                              'targetFile': 'Results/GWN/' + str(forecast_len) + ' Hour Forecast/Targets/' + 'targets_'
                                            + str(k) + '.pkl',
                              'trainLossFile': 'Results/GWN/' + str(forecast_len) + ' Hour Forecast/Loss/train_loss_' + 
                                            str(k) + '.csv',
                              'validationLossFile': 'Results/GWN/' + str(forecast_len) + ' Hour Forecast/Validation/validation_loss_' + 
                                            str(k) + '.csv',
                              'modelFile': 'Garage/Final Models/GWN/' + str(forecast_len) + ' Hour Models/model_split_' 
                                            + str(k),
