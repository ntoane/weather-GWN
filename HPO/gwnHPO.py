...
def train_model(args, data, split, supports, adj_init, model_file):
    ...
    x_validation, y_validation = util.sliding_window(scaler.transform(validate_data), args.lag_length, args.seq_length, split, 1)
...
def hpo(increment, args):
    ...
    for i in range(args.num_configs):
        print('This is the ', i + 1, 'th configuration: \n')
        ...
        for k in range(num_splits):
            ...
            # supports: list of adjacency matrices
    ...
    print('Done! The best configuration is: ', best_cfg, ' with an MSE of: ', best_mse)
    f.write('This is the best configuration ' + str(best_cfg) + ' with an MSE of ' + str(best_mse) + '\n' +
            'Batch Size - ' + str(best_cfg[0]) + '\n' +
            'Dropout - ' + str(best_cfg[1]) + '\n' +
            'Hidden Units - ' + str(best_cfg[2]) + '\n' +
            'Input sequence - ' + str(best_cfg[3]) + '\n' +
            'Number of layers - ' + str(best_cfg[4]) + '\n' +
            'Epoch - ' + str(best_cfg[5])
    )
    f.close()
